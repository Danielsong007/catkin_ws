...
class DepalletizingHelper(object):
    ...
    def __init__(self, ):
        ...
        self.can_trigger_photo_ = False
        self.store_DI_state_ = False
        self.fetch_DI_state_ = False
        ...
        self.get_robot_tcp_client = self._create_client(
        'robot/get_group_pose', GetGroupPose, False)
        ...
        self._set_init_flag_srv = rospy.Service(
            'set_init_flag', SetInitFlag, self._set_init_flag_handle
        )

        self._set_trigger_photo_srv = rospy.Service(
            'set_trigger_photo', SetTriggerPhoto, self._set_trigger_photo_handle
        )
    ...
    def _set_trigger_photo_handle(self, req):
        resp = SetTriggerPhotoResponse()
        self.can_trigger_photo_ = True
        rospy.loginfo('allow to trigger photo rightnow')
        resp.result_status = resp.SUCCEEDED
        return resp

    def _set_init_flag_handle(self, req):
        resp = SetInitFlagResponse()
    ...
    def _execute_planning_handle(self, req):
        ...
        # if pick from A or B, need to move linear joint lower giving space for photoing
        if self.aubo_pose_ == 'A' or self.aubo_pose_ == 'B':
            post_middle_pose[0] = 1.0
        # if pick from C or D, need to move linear joint higher giving space for photoing
        elif self.aubo_pose_ == 'C' or self.aubo_pose_ == 'D':
            post_middle_pose[0] = 1.6
    ...
    def _sense_object_pose_handle(self, req):
        ...
        # wait for robot and aubo initialization, otherwise do not allow to photo
        while not rospy.is_shutdown() and not self.is_initialized_:
            rospy.sleep(0.1)
        ...
        # enter this sense_object_pose_handle function means captured once, do not allow repeaded capture
        self.can_trigger_photo_ = False
                ...
                if self.get_object_pose_client in self.enabled_clients:
                    ...
                    if get_object_pose_resp.result_status == get_object_pose_resp.FAILED:
                        ...
                        if get_object_pose_resp.pose_amount == get_object_pose_resp.SINGLE:
                            ...
                            # if no optimal one, return a fake zero pose
                            if np.linalg.det(pose_detected_transformed_matrix_optimal) == 0:
                                resp.pose.position.x = 2333
                                resp.pose.position.y = 0
                                resp.pose.position.z = 0
                                resp.pose.orientation.x = 0
                                resp.pose.orientation.y = 0
                                resp.pose.orientation.z = 0
                                resp.pose.orientation.w = 1
                                resp.result_status = resp.SUCCEEDED
                                resp.pose = obj_pose
                                ...
                                obj_pose.position.y = pose_detected_transformed.position.y
                                obj_pose.position.z = pose_detected_transformed.position.z                
                        # output the next target pose
                        resp.result_status = resp.SUCCEEDED
                        resp.pose = obj_pose
                        return resp

                # if no get object pose eye node, stop right away        
                else:
                    rospy.logerr('Get object pose client is not enabled, please make sure the box node is running')
                    resp.result_status = resp.FAILED
                    return resp

        # if no smart eye node, stop right away
        else:
            rospy.logwarn('Get point cloud client is not enabled, please make sure smart eye node is running')
            resp.result_status = resp.FAILED
            return resp
    ...
    def _store_detected_info_handle(self, req):
        # store the newest detection info into member virable self.target_pose_
        self.target_pose_ = req.pose
        rospy.sleep(0.1)
        self.store_DI_state_ = True

        # wait for robot to fetch this newest detection info
        while not rospy.is_shutdown() and not self.fetch_DI_state_:
            rospy.sleep(0.1)
        self.fetch_DI_state_ = False

        # wait for robot to tell whether can take photo
        resp = StoreDetectedInfoResponse()
        while not rospy.is_shutdown() and not self.can_trigger_photo_:
            rospy.sleep(0.1)
    ...
    def _fetch_detected_info_handle(self, req):
        ...
        # wait for the newest detected info to be saved in self.target_pose_.
        while not rospy.is_shutdown() and not self.store_DI_state_:
            rospy.sleep(0.1)

        # robot gets the newest detection info for motion planning
        resp.pose.position.x = self.target_pose_.position.x
        resp.pose.position.y = self.target_pose_.position.y
        resp.pose.position.z = self.target_pose_.position.z
        resp.pose.orientation.x = self.target_pose_.orientation.x
        resp.pose.orientation.y = self.target_pose_.orientation.y
        resp.pose.orientation.z = self.target_pose_.orientation.z
        resp.pose.orientation.w = self.target_pose_.orientation.w
        print('pose fetched is {}', format(resp.pose))

        # whether need to change photo pose
        if resp.pose.position.x == 2333:
            ...
            if self.aubo_pose_ == 'C':
                rospy.logwarn('%s district has no boxes been detected and will no longer take picture' % self.aubo_pose_)
            ...
            self.store_DI_state_ = False
            self.fetch_DI_state_ = True
            self.can_trigger_photo_ = True
            resp.result_status = resp.FAILED
            return resp

        # no need to change pose
        self.store_DI_state_ = False
        self.fetch_DI_state_ = True
        resp.result_status = resp.SUCCEEDED
        return resp
...
if __name__ == "__main__":
    try:
        rospy.init_node('roport_depalletizing_helper_parallel')
