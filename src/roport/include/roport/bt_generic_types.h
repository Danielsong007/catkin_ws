...
namespace BT
{
  ...
  template <> inline PoseArray convertFromString(StringView str) {
    ...
    std::cout << "we have " <<len<<" waypoints in total" << std::endl;
    PoseArray output{};
      // we detect str bu '{' and get its real value
    for (auto p_str : poses_str) {
//      if(p_str.find('{')!=p_str.npos){
//        char pstr[20];
//        std::size_t length=p_str.copy(pstr,p_str.size()-2,2);
//        pstr[length]='\0';
//        std::string pstr1=pstr;
//        Pose p{};
//        Blackboard bb;
//        bool x = bb.get(pstr1,p);
