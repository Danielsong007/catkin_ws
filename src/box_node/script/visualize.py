...
# raw_pts: o3d.geometry.PointCloud
# selected_box_pts: np.array
def visualize_multi_pose_in_raw_pts(raw_pts, selected_box_pts, R_list, t_list):
    viz_transformation = np.identity(4, np.float32)
    viz_transformation[0, 0] = -1.0
    viz_transformation[2, 2] = -1.0
    ...
    for idx in range(len(R_list)):
        ...
        pose_axis_pcd.transform(transformation)
        pose_axis_pcd.transform(viz_transformation)
        pose_axis_pcd_list.append(pose_axis_pcd)
        
    # rotate the scene for better visualization
    box.transform(viz_transformation)
    raw_pts.transform(viz_transformation)

    o3d.visualization.draw_geometries([box] + [raw_pts] + pose_axis_pcd_list)
...
def visualize_pose_in_raw_pts_one_box(raw_pts, selected_box_pts, R, t, overlap, grasper_box, lines):
    ...
    transformation[:3, :3] = R
    transformation[:3, 3] = t
    pose_axis_pcd = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.400, origin=[0, 0, 0])
    pose_axis_pcd.transform(transformation)

    # rotate the scene for better visualization
    box.transform(viz_transformation)
    raw_pts2 = o3d.geometry.PointCloud()
    raw_pts2.points = o3d.utility.Vector3dVector(raw_pts)
    raw_pts2.transform(viz_transformation)
    pose_axis_pcd.transform(viz_transformation)
    ...
    o3d.visualization.draw_geometries([box] + [raw_pts2] + [] + [pose_axis_pcd] + [line_set])
...
def visualize_pose_in_raw_pts_two_box(raw_pts, selected_box_pts, R, t, grasper_box, lines, box_range, box_range_lines):
    ...
    transformation[:3, :3] = R
    transformation[:3, 3] = t
    pose_axis_pcd = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.100, origin=[0, 0, 0])
    pose_axis_pcd.transform(transformation)
